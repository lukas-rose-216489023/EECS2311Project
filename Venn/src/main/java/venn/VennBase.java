		...
		textAdder.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				if (event.getButton() == MouseButton.PRIMARY) {
					createTextBox(pane, textAdder, "New Text Box", circleL, circleR, intersection, leftCircle, rightCircle, p);
				}
			}
		});
				...
				finish.setOnMouseClicked(new EventHandler<MouseEvent>() {
					...
					public void handle(MouseEvent event) {
						...
						for (int i = 0; i < topics.size(); i++) {
							createTextBox(pane, textAdder, topics.get(i), circleL, circleR, intersection, leftCircle, rightCircle, p);
						}
						dialog.close();
					}
...
//		}
	...
	protected void createTextBox(Pane pane, Button textAdder, String text, Circle circleL, Circle circleR, Anchor intersection, Anchor leftCircle, Anchor rightCircle, Points p) {
		int stackable = (int) (pane.getHeight() / (textAdder.getHeight()-10)) -2;


		//Text box properties

		Button box = new Button(text);
